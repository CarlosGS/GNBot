...
#cv.NamedWindow("Perspective reference points")
...
space = 30.
it = 0
for x in np.linspace(-50, 50, num=7, endpoint=True):
    for y in np.linspace(-50, 50, num=7, endpoint=True):
        if it%2 == 0:
...
lastPointTime = datetime.datetime.now()

print("pointList = " + str(pointList))

def message_received(datain):
    global robot_dest_addr_long, robot_battery, current_logging, data, robot_speed, turn_dir, backing_up, waiting, robotAngle, robotPosX, robotPosY, lastPointTime
    ...
    if current_logging:
        ...
        if (timeStamp-lastPointTime).total_seconds() > pointDelay:
            pointList.pop(0)
...
while 1:
    ...
    if M != None:
        ...
        thresh_angle = cv2.inRange(roi_big,np.array((good_color_angle[0]-15, good_color_angle[1]-30, 180)), np.array((good_color_angle[0]+5, good_color_angle[1]+20, 255)))
...
saveToFile(data,"./","logged_data_nose.p")
