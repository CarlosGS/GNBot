...
def exitCallback():
    print("Exiting...")
    saveToFile(log,"./data/",str(datetime.now())+".p")
    print("Closing connection...")
    robot.send("Init:ByeBye!\n") # This will stop the robot
    robot.disconnect()
atexit.register(exitCallback)

print("Press enter to start the battery test!")
...
while 1:
    vals = read_vals()
    #print(str(datetime.now()) + " -> " + str(vals))
    if len(vals) > 1:
        try:
            timestamp = datetime.now()
            if count > 60*60: # Backup each ~hour
                saveToFile(log,"./data/",str(timestamp)+".p")
                count = 0
#            hr_hex = vals[2]
#            hr = int(hr_hex[0])*16 + int(hr_hex[1])
            datapoint = {}
            datapoint["timestamp"] = timestamp
            datapoint["hr"] = int(vals[1], 16)
            #print(str(timestamp) + " -> " + str(datapoint["hr"]))
            datapoint["all"] = [int(val,16) for val in vals]
            log.append(datapoint)
            count += 1
            time.sleep(0.2)
        except:
            print(str(datetime.now()) + " Error except!")
            continue
